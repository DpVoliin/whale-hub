...
| `.github/workflows/` | CI：片段合并 + 结构断言 + 产物等价 + 脱敏回归 + lint |
| `docs/` | 架构取舍、隐私设计、路线图、扩展契约 |
...
| **有料程度** | 雨≥60% / 电量≤20% 未充 / 蓝牙设备≤20% / 有课 / 快递订单 / 游戏≥60 分 / 屏幕≥5 小时，每项 +1 分；**相对他自己历史反常**再加分（见下）；**≥3 分 ×0.6（说勤点）**，**0 分 ×1.4（少说）** |
...
| **反馈学习** | 挂件右键 ✓/✗ → 中枢 `/feedback` → 说话层喂给 Thompson 后验 | 每日上限随之重算（θ=0.1→5 句，θ=0.9→11 句；样本<4 就不动她） |
