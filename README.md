...
说话层最危险的故障不是崩溃，而是**沉默失败** —— 看起来一切正常，只是什么都不做。
2026-09-24 一天里连着出了四个（模型空回复 / 判重跨天 / 回执失败重发 / 补发过期提醒），
每一个都要翻几千行日志才能发现。所以这三道机制是**机制**，不是"记得看一眼"：

- **模型画像**（`speaker/model_profile.py`）—— 不同厂商的思考型/非思考型差异很大：
  参数名（`o1/o3/gpt-5` 只认 `max_completion_tokens`）、预算（思考型 2400 / 非思考型 400）、
  `temperature` 约束（o 系列只接受 1）。名字认不出的**保守按思考型**；响应里出现
  `reasoning_content` 而名字没标 → 当场改判。调用侧还有三层兜底：翻三倍重试 → 切备用模型 → 模板。
- **自检与看门狗** —— 启动时验「中枢可达 / token 有效 / 模型可达 / 状态目录可写」；
  运行中若**接口连错 ≥5、模型空 ≥5、活跃时段 ≥6 小时没开口且有料** → 她会发一条自查消息
  （每天最多一次，免打扰时段不判）。另有逐类计数覆盖全部 11 个"不说/丢弃"出口。
- **策略回归门禁**（`speaker/sim_week.py`）—— 调说话层**真函数**跑一周模拟（带假时钟，
  否则台账跨天不重置），把"时段错位 / 平均条数越界 / 免打扰失效"当不变量校验，
  破了就非 0 退出。CI 里每次都跑，`--check` 也可本地手动跑。
