...
- [x] **蓝牙外设电量**：耳机/手表低电提醒
- [x] **小设备接入（上行）**：`/api/mcu` 一行上报 + 内网中继（STM32/C51）
- [ ] ~~硬件设备（STM32 小屏 + 语音模块）~~ **已评估、短期不做**：
      接口侧已验证可行（`/mcu/inbox` + `/mcu/ack` 一行纯文本 · gb2312 直通中文 TTS · 可朗读化剥动作标注），
      成本约 90 元 + 需家里一台常开机器做中继；放弃原因：短期用不上，不为它增加维护面。
...
- [x] **压测合成数据 + 反事实调参**：`--density real` 造 180 天/41 万行（8.8 秒）；
      `stress_report.py` 量函数与接口耗时；`tune_gap.py` 用留出法回放挑阈值（v0.1.16）
